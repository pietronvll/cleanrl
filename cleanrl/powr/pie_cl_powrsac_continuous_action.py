...
import tyro
from contrastive_repr import SpectralConLoss
...
class ReprReplayBuffer(ReplayBuffer):
    def sample_contrastive(self, batch_size, env=None):
        ...
        next_obs = self.next_observations[batch_idx].reshape(
            -1, *self.next_observations.shape[2:]
        )
...
class ContrastRepr(nn.Module):
    def __init__(self, env, latent_dim: int = 256):
        super().__init__()
        ...
        self.reward_out = nn.Linear(latent_dim, 1, bias=False)
    ...
    def forward(self, s, a, next=False):
        ...
        if next:
            ...
            r_pred = self.reward_out(x)
        return x, r_pred


# ALGO LOGIC: initialize agent here:
class SoftQNetwork(nn.Module):
    def __init__(self, env, latent_dim=256):
        super().__init__()
        self.embedder = ContrastRepr(env, latent_dim=latent_dim)
        # World model
        self.register_buffer(
    ...
    def compute_covariances(self, rb: ReplayBuffer, actor: Actor, num_mean_samples=10):
        ...
        X, _ = self.embedder(b_obs, b_actions)
        ...
        for action in actions:
            ...
            emb, _ = self.embedder(b_next_obs, action, next=True)
            Y += emb
        Y /= num_mean_samples
    ...
    def fit_world_model(self, rb: ReplayBuffer, actor: Actor):
        C0, C1 = log_time(self.compute_covariances)(rb, actor)
        ...
        reward_weights = self.embedder.reward_out.weight.detach().T
        # Inplace evaluation of Id - discount*T_ridge
        T_ridge *= -args.gamma
        T_ridge.diagonal().add_(1)
        sol = torch.linalg.solve(T_ridge, reward_weights)
...
"""
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env(args.env_id, args.seed + i, 0, args.capture_video, run_name)
    ...
    repr_optimizer = optim.Adam(list(qf1.embedder.parameters()), lr=args.cont_lr)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            obs_repr, r_pred = qf1.embedder(data.observations, data.actions)
            with torch.no_grad():
                next_actions, _, _ = actor.get_action(data.next_observations)
            next_obs_repr, _ = qf1.embedder(
                data.next_observations, next_actions, next=True
            )
            ...
            if global_step % 100 == 0:
                ...
                sa_raw = (
                    torch.cat([data.observations, data.actions], dim=-1).cpu().numpy()
                )
                # print(sa_np.shape, r_np.shape, sa_raw.shape)
                writer.add_scalar("losses/repr_reward", reward_loss.item(), global_step)
                writer.add_scalar(
                    "losses/baseline_reward",
                    ridge_eval(ridge_raw, sa_raw, r_np),
                    global_step,
                )
