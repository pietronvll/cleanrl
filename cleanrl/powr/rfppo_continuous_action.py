...
class OrthogonalRandomFeatures(torch.nn.Module):
    def __init__(
        ...
        # create a linea layer to sack after the batch norm
        self.linear = nn.Linear(self.num_random_features, 1)
...
class Agent(nn.Module):
    def __init__(self, envs, rf_seed=0):
        super().__init__()
        state_dim = np.prod(envs.single_observation_space.shape)
        action_dim = np.prod(envs.single_action_space.shape)
        self.critic = OrthogonalRandomFeatures(
            state_dim,
            num_random_features=args.num_rfs,
            length_scale=sqrt(state_dim),
            rng_seed=rf_seed,
        )
