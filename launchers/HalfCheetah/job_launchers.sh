    ...
    for critic_layers in $critic_layers_arr; do
      for learning_starts in $learning_starts_arr; do
        for reward_weight in $reward_weight_arr; do
          qsub -v REP_LOSS="$rep_loss",CRITIC_LAYERS="$critic_layers",LEARNING_STARTS="$learning_starts",REWARD_WEIGHT="$reward_weight",CRITIC_TRAINING="$critic_training" launchers/HalfCeetah/repl_sac.sh
        done
      done
    done
  done
